...
from src.graph_tab import GraphTab

class PlotAttitudeUpdated(GraphTab):
    def setup(self):
        # This method is called by GraphTab.__init__ after self.data_source is available
        # if self.data_source is set before super().__init__ in this class.
        self.title = f"Attitude ({self.data_source})"

    def __init__(self, master, data, args, data_source='BR', save_animation_on_start=False):
        # --- Setup required by self.graph() and self.setup() --- 
        self.data_source = data_source # Needs to be set before super for setup() to use it
        self.data = data      # Needed for _load_attitude_data
        self.title = f"Attitude ({self.data_source})" # Set initial title, setup() will confirm.
        
        # Initialize attributes used in graph() or its callees
        self.quaternions = []
        self.rotations = []
        ...
        # Load data required by graph()
        self._load_attitude_data()
        if self.quaternions:
            self._calculate_rotations()
        # --- End Setup required by self.graph() --- 

        # Now call super().__init__. It will call self.setup(), then self.graph().
        super().__init__(master, data, args)

        # --- Other initializations (not directly needed by the first graph()/setup() call) ---
        self.output_dir = args.get('attitude_output_dir', os.path.join('.', 'data_csv', 'animations'))
        os.makedirs(self.output_dir, exist_ok=True)

        # Check if animation should be saved (self.ani would have been set by graph() via super())
    ...
    def graph(self): # Overrides GraphTab.graph()
        self.fig.clear() 
        ...
        if not self.quaternions or not self.rotations:
            self.ax.text2D(0.5, 0.5, f"No data/rotations for {self.data_source}", 
                           transform=self.ax.transAxes, ha="center", va="center")
            # self.canvas.draw() # Removed: GraphTab handles initial draw
            return
        ...
        # self.canvas.draw() # Removed: GraphTab handles initial draw, animation updates itself
    ...
    def save_animation_to_file(self):
        ...
        try:
            ...
            self.ani.save(output_path, writer=writer, dpi=150,
                          progress_callback=lambda i, n: print(f'Saving frame {i+1}/{n} for {self.data_source}', end="\r"))
...
# Removed if __name__ == "__main__": block as this is now a module for avionics_data.py 